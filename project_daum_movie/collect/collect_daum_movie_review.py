...
# ** Selenium을 사용하는 이유?
#  - Request는 현재 URL의 정적 페이지 소스코드만 수집 가능
...
def review_collector(movie_code, last_date):
    ...
    chlick_cnt = math.ceil((num_review - 10) / 30)

    # 7. Selenium을 통해서 "평점 더보기" 클릭
    for i in range(chlick_cnt):
        # "평점 더보기" 클릭
    ...
    for item in review_list:
        ...
        if review_date == "조금전":
            ...
            # 1분전~59분전 → "분전"
            reg_minutes = int(re.sub(r"[^~0-9]", "", review_date))
            review_date = datetime.now() - timedelta(minutes=reg_minutes)
            review_date = review_date.strftime("%Y. %m. %d. %H:%M")
